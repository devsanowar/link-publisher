...
@section('title', 'Edit Seo Service')
@push('styles')
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 mt-3">
                <div class="card">
                    <div class="card-header">
                        <h4 class="">Edit Seo Service<span><a href="{{ route('seo_service.index') }}" class="btn btn-primary text-white text-uppercase text-bold right"> All Packages </a></span></h4>
                    </div>
                    <div class="body">
                        <form id="editSeoServiceForm" class="form-horizontal" method="POST">
                            @csrf
                                ...
                                <div class="form-group">
                                    <div class="" style="border: 1px solid #ccc">
                                       <select name="package_type" id="" class="form-control show-tick">
                                            ...
                                            <option value="custom" {{ $package->package_type == 'custom' ? 'selected' : '' }}>Customized</option>
                                       </select>
                                    </div>
                                </div>
                            </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#editSeoServiceForm').on('submit', function(e) {
                e.preventDefault();
                let formData = $(this).serialize();

                $.ajax({
                    type: 'POST',
                    url: "{{ route('seo_service.update') }}",
                    data: formData,
