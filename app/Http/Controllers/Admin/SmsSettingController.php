...
class SmsSettingController extends Controller
{
    ...
    public function sendCustomSms(Request $request)
    {
        ...
        $totalMessageParts = ceil($totalCharacters / 160);

        foreach ($mobileNumbers as $mobile) {
            if (!preg_match('/^\d{10,15}$/', $mobile)) {
                Log::warning("Invalid mobile number skipped: $mobile");
                continue;
            }
            ...
            try {
                ...
                if (!isset($responseData['status']) || $responseData['status'] != 'success') {
                    Log::error("SMS failed for $mobile: " . json_encode($responseData));
                    continue;
                }
                ...
                try {
                    $log = SmsLog::create([
                        ...
                        'delivery_report' => $responseData['status'] ?? 'unknown',
                    ]);

                    Log::info('Log Inserted: ', $log->toArray());
                } catch (\Exception $e) {
                    Log::error('Log Insert Exception: ' . $e->getMessage());
                }
            } catch (\Exception $e) {
                Log::error("Exception for $mobile: " . $e->getMessage());
                continue;
            }
        }
